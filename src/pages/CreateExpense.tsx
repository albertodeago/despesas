...
import { SessionContext, GroupsContext, CategoriesContext, UserId } from "../contexts"
...
export function CreateExpense() {
    ...
    const [availableCategories, setAvailableCategories] = useState<ExpenseCategory[]>(() => categories.filter(c => c.group_id === selectedGroup.id))
    const [selectedCategory, setSelectedCategory] = useState<ExpenseCategory>(availableCategories[0])

    // const [payers, setPayers] = useState<Object>({})
    const owner = session.user!.id

    const createExpense = async function(params: {name: string, amount: number, group: ExpenseGroup, category: ExpenseCategory, owner: UserId}) {
        setLoading(true)
    ...
    const onCategoryChange = function(evt: React.ChangeEvent<HTMLSelectElement>) {
    ...
    return (
        <div>
            ...
            <div>
                ...
                <select name="categories" id="categories" value={JSON.stringify(selectedCategory)} onChange={(evt) => onCategoryChange(evt) }>
                    {
            ...
            <div>
                <button
                    ...
                    onClick={() => createExpense({ name, amount, group: selectedGroup, category: selectedCategory, owner })}
                    disabled={loading}
                >
